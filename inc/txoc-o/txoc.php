...
function idesign_txoc_admin_js() {
	...
	/*
	wp_localize_script('idesign-txoc-admin-js', 'txoc_content',
			array(
				'stage1'     	=> $intropop_content,					
				'stage2'    	=> idesign_customizer_demolist(),
			)
	);
	*/	
...
function idesign_customizer_intropop () {
    ...
    if ( !get_user_meta($user_id, 'idesign_ignore_txoc_notice_006') ) {
	...
	$intropop_content .= '<h4>'. esc_html__('Please Install accompanying plugin Templatesnext ToolKit to take full advantages of all the features.', 'i-design') .'</h4>';		
	$intropop_content .= '</div>';
	$intropop_content .= '<div class="tx-features">';
	$intropop_content .= '<h1>'. esc_html__('Main Features', 'i-design') .'</h1>';
	$intropop_content .= '<ul>';
	$intropop_content .= '<li>'. esc_html__('Ready To Import Pages.', 'i-design') .'</li>';
	...
	if ( !get_user_meta($user_id, 'idesign_ignore_txoc_notice_006') ) {
		$activation_button .= '<a class="never-show button button-hero" href="?idesign_txoc_notice_ignore_006=0">' . __( 'Never Show This Popup', 'i-design' ) . '</a>';	
	}
...
function idesign_customizer_demolist () {
	...
	foreach ( $demolist as $vals ) {
		...
		foreach ( $vals['categories'] as $class ) {
			$intropop_classes .= 'nx-' . strtolower($class) . ' ';
		}
	...
	$intropop_content .= '</div>';
	if ( !get_user_meta($user_id, 'idesign_ignore_txoc_notice_006') ) {
		$intropop_content .= '<div class="intropop-bottom"><a class="never-show button" href="?idesign_txoc_notice_ignore_006=0">' . __( 'Never Show This Popup', 'i-design' ) . '</a></div>';	
	}	
	$intropop_content .= '</div>';	
	...
	foreach ( $demolist as $vals ) {
		...
		if ( in_array( "Free", $vals['categories']) ) {
					...
					foreach ( $required_plugins_list as $plugin ) {				
						...
						{				
							...
							} elseif( idesign_is_plugin_installed($pluginTitle) == false )
							{
								...
								$intropop_content .= '<a class="button activate-txoc-now button-primary" data-active-lebel="' . __( 'Activating...', 'i-design' ) . '" data-slug="' . $plugin['slug'] . '" href="' . esc_url( $pluginLink ) . '" aria-label="Activate ' . $plugin['slug'] . '" style="display: none;">Activate</a>';
								$intropop_content .= '<a href="#" class="button disabled txoc-active" style="display: none;">' . __( 'Plugin installed and active', 'i-design' ) . '</a>';  
							} else
							{
								$intropop_content .= '<a class="button activate-txoc-now button-primary" data-active-lebel="' . __( 'Activating...', 'i-design' ) . '" data-slug="' . $plugin['slug'] . '" href="' . esc_url( $pluginLink ) . '" aria-label="Activate ' . $plugin['slug'] . '">Activate</a>';
								$intropop_content .= '<a href="#" class="button disabled txoc-active" style="display: none;">' . __( 'Plugin installed and active', 'i-design' ) . '</a>';  
							}
			...
			$intropop_content .= '<h4>'. esc_html__('This demo site is only available with premium theme ', 'i-design' ) . '<a href="' . esc_url('//templatesnext.org/ispirit/landing/') . '" target="_blank">' . esc_html__( 'I-SPIRIT', 'i-design' ) . '</a>' . '</h4>';
			$intropop_content .= '</div>';				
		}			
		...
		if ( in_array( "Free", $vals['categories']) ) {
			...
			$intropop_content .= '<a href="' . esc_url('//templatesnext.org/ispirit/landing/') . '" class="button button-hero button-primary go-prem" data-item-number="'.$item_counter.'" target="_blank" data-required-plugins="'.$intropop_plugins.'" >'. esc_html__( 'Go Premium', 'i-design' ) .'</a>';
		}		
	...
	$intropop_content .= '<a href="'.site_url().'/wp-admin/customize.php?idesign_txoc_notice_ignore_006=0" class="button" target="_self" >'. esc_html__( 'Customize', 'i-design' ) .'</a>';
...
add_action('admin_init', 'idesign_txoc_notice_ignore_006');
function idesign_txoc_notice_ignore_006() {
    global $current_user;
	$user_id = $current_user->ID;

	if ( isset($_GET['idesign_txoc_notice_ignore_006']) && '0' == $_GET['idesign_txoc_notice_ignore_006'] ) {
    	add_user_meta($user_id, 'idesign_ignore_txoc_notice_006', 'true', true);
    }
	
	if ( isset($_GET['activated']) && $_GET['activated'] == 'true' ) {
    	add_user_meta($user_id, 'idesign_txoc_show_pops', 1, true);
    }	
}
